...
package com.mattism.http.xmlrpc
{
	...
	implements MethodCall
	{
		...
		//added by metscher 10.08.2008 - 21:27
		...
		private function createParamsNode( parameter:Object ):XML {
			...
			if ( typeof parameter == "object") {
				...
				if (parameter.type == XMLRPCDataTypes.STRUCT) {
					...
					TypeNode = <struct />;
					// block (modified by Johannes Metscher 11.08.2008 - 18:44)
					// now it's possible to parse an array which contain structs or arrays 
					for (var xName in parameter.value) {
						var x = parameter.value[xName];
						
						var MemberNode:XML = <member />;
					
						MemberNode.appendChild(<name>{xName}</name>);
						if( x is String ){
							MemberNode.appendChild(<value><string>{parameter.value[xName]}</string></value>);
						}
						else if( x is int ){
							MemberNode.appendChild(<value>{parameter.value[xName]}</value>);
						}
						else if( x is Object ){
							MemberNode.appendChild(createParamsNode( x ));
						}	
						// end block
